...
# Configurer le format du logger et le niveau de log INFO
logging.basicConfig(format='%(asctime)s %(levelname)s - %(message)s', level=logging.INFO)

# Configurer le niveau de log ERROR pour le logger "requests"
logging.getLogger("requests").setLevel(logging.ERROR)

#################### TEMPLATES DEFINITION HERE ######################
templatesFolder = "payloads"
with open("./lib/fuploader/templates.json", "r", encoding='utf-8') as fd:
...
if args.randomUserAgent:
    with open("./lib/fuploader/user-agents.txt","r") as fd:
        nb = 0
        for l in fd:
            nb += 1
        fd.seek(0)
...
mimeFile = "./lib/fuploader/mimeTypes.basic"
...
with open("./lib/fuploader/techniques.json", "r") as rawTechniques:
...
for template in templates:
    with open(os.path.join("./lib/fuploader/"+(templatesFolder + "/" + template["filename"])), 'rb') as templatefd:
